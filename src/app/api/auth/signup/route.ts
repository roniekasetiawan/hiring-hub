...
type Body = { email: string; password: string; full_name?: string; role?: 'admin'|'recruiter'|'applicant' };

export async function POST(req: NextRequest) {
    const { email: rawEmail, password, full_name, role = 'applicant' } = (await req.json()) as Body;
    const email = (rawEmail || '').trim().toLowerCase();

    if (!email || !password) return error(null, 400, 'Email & password are required');
    if (password.length < 6) return error(null, 422, 'Password must be at least 6 characters');
    ...
    const token = await signToken({ sub: data.id, email: data.email, role: data.role });
